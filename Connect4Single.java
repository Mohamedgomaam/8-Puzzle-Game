...
public static String typeofsearch=" ";
	...
	public static int minimax(int[][] checkBoard, int x, int y, int alpha,int beta, int depth, boolean playerturn) {
		int temp = check(checkBoard, x, y);
		//check the board if there is anyone winning
		if (temp == 1) {
			return -INF;
		...
		if (playerturn) {
			...
			for (int i = 0; i < column; i++) {
				...
				// get the value of that number from calling another minimax
				int val = minimax(newBoard, checkBoard[row][i], i, alpha, beta,depth - 1, !playerturn);
	...
	public static int check(int[][] checkBoard, int x, int y) {
		...
		// if in a colum , there is a connected 4
		for (int i = x - 3; i <= x; i++) {
	...
	int choise=JOptionPane.showOptionDialog(null,"select Type of search ", "TYPE", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, myicon, types,0);
